...
public class Main {	
	...
	public static void main(String[] args) {
		Rain(); // Pluie de caractères verts
		System.out.println("BIENVENUE DANS LA MATRICE");
		
		//===================================================================
		// Membres et vaisseaux ajoutés par défaut pour tester plus rapidement
		try {
			flotte.addUniquePersonnel(new OperateurSION("Louis", false, 19, "Apprenti", "Pilote"));
			flotte.addUniquePersonnel(new MembreLibere("Morpheus", false, 19, "Général"));
			flotte.addUniquePersonnel(new MembreLibere("Neo", false, 19, "Lieutenant"));
	...
	// Gère l'exfiltration d'un membre de la matrice
		...
		for (int i=0; i<NB_LIGNES; i++) {
			for (int j=0; j<NB_COLONNES; j++) {
				if (long_colonnes[j] >= i) {
					// Affiche selon la taille de la colonne définie plus haut
